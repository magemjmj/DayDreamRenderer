...
namespace daydreamrenderer
{
    ...
    public class DaydreamRenderer : MonoBehaviour
    {
        ...
        private bool m_supportfloatRenderTexture = true;

        private bool m_supportRenderTexture_Depth = false;
        private bool m_supportRenderTexture_ARGBHalf = false;
        private bool m_supportRenderTexture_RGHalf = false;
        private bool m_supportRenderTexture_Shadowmap = false;
        private bool m_supportRenderTexture_ARGBFloat = false;
        private bool m_supportRenderTexture_RGFloat = false;
        private bool m_supportRenderTexture_ARGB32 = false;
    ...
    void OnGUI()
    {
        ...
        public void Update()
        {
            ...
            if (Camera.main != null && m_maxShadowCasters > 0)
            {
                if (m_shadowComposite == null)
                {
                    buildCompositeBuffer();
                ...
                {
                    if (m_supportfloatRenderTexture)
                ...
                if (ShadowCaster.s_shadowCastingObjects != null)
                {
                    ...
                    if (m_maxShadowCasters == 1)
                    {
                        ...
                        */
                        if (ShadowCaster.s_shadowCastingObjects[0].Count > 0)
                        {
                            float blend = buildShadowCamera(0);
                            if (blend > 0.0f)
                            {
        ...
        {
            RenderTextureFormat renderTextureFormat = m_supportRenderTexture_RGHalf ? RenderTextureFormat.RGHalf : RenderTextureFormat.ARGBHalf;
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                m_shadowmap[i] = new RenderTexture(m_currentWidth, m_currentHeight, 16, renderTextureFormat);
                m_supportfloatRenderTexture = m_shadowmap[i].Create();
            }

            m_workBuffer = new RenderTexture(m_currentWidth, m_currentHeight, 0, renderTextureFormat);
            m_supportfloatRenderTexture = m_workBuffer.Create();
        ...
        private void checkSupportRenderTexture()
        {
            m_supportRenderTexture_Depth = SystemInfo.SupportsRenderTextureFormat(RenderTextureFormat.Depth);
            m_supportRenderTexture_ARGBHalf = SystemInfo.SupportsRenderTextureFormat(RenderTextureFormat.ARGBHalf);
            m_supportRenderTexture_RGHalf = SystemInfo.SupportsRenderTextureFormat(RenderTextureFormat.RGHalf);
            m_supportRenderTexture_Shadowmap = SystemInfo.SupportsRenderTextureFormat(RenderTextureFormat.Shadowmap);
            m_supportRenderTexture_ARGBFloat = SystemInfo.SupportsRenderTextureFormat(RenderTextureFormat.ARGBFloat);
            m_supportRenderTexture_RGFloat = SystemInfo.SupportsRenderTextureFormat(RenderTextureFormat.RGFloat);
            m_supportRenderTexture_ARGB32 = SystemInfo.SupportsRenderTextureFormat(RenderTextureFormat.ARGB32);

            m_supportfloatRenderTexture = m_supportRenderTexture_RGHalf || m_supportRenderTexture_ARGBHalf;
