...
namespace daydreamrenderer.FlatBuffers
{
	public class FlatBufferBuilder
	{
		...
		public StringOffset CreateString(string s)
		{
			...
			Encoding.UTF8.GetBytes(s, 0, s.Length, _bb.Data, _space -= byteCount);
		...
		public int EndObject()
		{
			...
			AddInt(0);
			int offset = Offset;
			for (int num = _vtableSize - 1; num >= 0; num--)
			{
			...
			AddShort((short)((_vtableSize + 2) * 2));
			int num2 = 0;
			for (int i = 0; i < _numVtables; i++)
			{
				int num3 = _bb.Length - _vtables[i];
				int space = _space;
				short @short = _bb.GetShort(num3);
				if (@short == _bb.GetShort(space))
				{
					int num4 = 2;
					while (num4 < @short)
					{
						if (_bb.GetShort(num3 + num4) == _bb.GetShort(space + num4))
						{
							num4 += 2;
							continue;
						}
		...
		{
			byte[] array = new byte[_bb.Data.Length - _bb.Position];
			Buffer.BlockCopy(_bb.Data, _bb.Position, array, 0, _bb.Data.Length - _bb.Position);
			return array;
		}
